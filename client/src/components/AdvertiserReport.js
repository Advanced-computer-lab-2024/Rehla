...
import { fetchAllSalesReportsemail, getAdvertiserProfile, fetchActivityReport, filterActivityAttendeesByMonth, calculateActivityRevenue, fetchFilteredAdvertiserSalesReport  } from '../services/api';
...
const AdvertiserReport = () => {
    ...
    const [filteredactivityReport, setFilteredactivityReport] = useState(null); // State for the filtered report
    ...
    const handleViewactivityReport = async () => {
        try {
            ...
            setErro("An error occurred while fetching the activity report.");
            console.error(err);
        }
    };
    ...
    const handleFilterFetchSalesReports = async () => {
        try {
            setLoading(true);
            const email = localStorage.getItem('email');
            const reports = await fetchFilteredAdvertiserSalesReport(email, activityFilter, startDate, endDate, month); // Send filters to API
    ...
    const handleFetchSalesReports = async () => {
        try {
            ...
            setMessagee('Error fetching activity sales reports.');
            console.error(err);
        }
    };
        ...
        <div>
            <div className="w-full mx-auto px-6 py-1 bg-black shadow flex flex-col sticky z-50 top-0">
                <div className="flex items-center">                
                    ...
                    <nav className="flex space-x-6">
                        ...
                        <Link to="/AdvertiserHome/AdvertiserGuideReport" className="text-lg font-medium text-logoOrange hover:text-blue-500">
                            Reports
                        </Link>
                    </nav>
            ...
            <div className="p-6 bg-gray-50 min-h-screen">
                ...
                <div className="bg-white p-6 rounded-md mb-8">
                    ...
                    <div className="flex flex-wrap gap-4">
                        <div className="w-full sm:w-1/5">
                            <label htmlFor="activityFilter" className="block font-medium mb-1">activity:</label>
                            <input
                                type="text"
                                id="activityFilter"
                    ...
                    <table className="w-full border-collapse border border-gray-200">
                        <thead>
                            <tr className="bg-gray-100">
                                <th className="border border-gray-200 p-3 text-left">Activity</th>
                        ...
                        <tbody>
                            {salesReports.map((report) => (
                                <tr key={report.Report_no} className="hover:bg-gray-50">
                                    <td className="border border-gray-200 p-3">{report.Activity}</td>
                ...
                <div className="mb-4 flex items-end gap-2">
                    ...
                    <button
                        onClick={handleFilteractivityByMonth}
                    ...
                    {activityReport && (
                    <div>
                        <h2 className="text-xl font-semibold mb-4">activity Report</h2>
                        <table className="w-full border-collapse border border-gray-200">
                        <thead>
                            <tr className="bg-gray-100">
                            <th className="border border-gray-200 p-2 text-left">activity Name</th>
                        ...
                        <tbody>
                            ...
                            <tr key={index} className="hover:bg-gray-50">
                                <td className="border border-gray-200 p-2">{activity.activityName}</td>
                                <td className="border border-gray-200 p-2">
                                {new Date(activity.date).toLocaleDateString()}
                                </td>
                                <td className="border border-gray-200 p-2">{activity.attendeesCount}</td>
                            </tr>
                            ))}
                        </tbody>
                        </table>
                        <p className="mt-4 font-semibold">
                        <strong>Total Attendees:</strong> {activityReport.totalAttendees}
                        </p>
                    </div>
                    )}
                    ...
                    <div className="mt-6">
                        <h2 className="text-xl font-semibold mb-4">Filtered activity Report</h2>
                        <table className="w-full border-collapse border border-gray-200">
                        <thead>
                            <tr className="bg-gray-100">
                            <th className="border border-gray-200 p-2 text-left">activity Name</th>
                        ...
                        <p className="mt-4 font-semibold">
                        <strong>Total Attendees:</strong> {filteredactivityReport.totalFilteredAttendees}
                        </p>
                    </div>
                    )}
