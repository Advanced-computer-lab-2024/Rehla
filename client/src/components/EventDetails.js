...
const EventDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      try {
        ...
        // Fetch the attended status from localStorage
        const attended = localStorage.getItem(`${selectedType}_${selectedName}_attended`);
    ...
    return (
      <div className="flex items-center">
        {[...Array(maxRating)].map((_, index) => (
          <span
            key={index}
            className={`text-3xl cursor-pointer ${index < currentRating ? "text-yellow-500" : "text-gray-300"}`}
  ...
  return (
    ...
    <div className="min-h-screen flex flex-col items-center">
      <header className="NavBar flex items-center justify-between p-4 bg-brandBlue shadow-md w-full">
        ...
        <nav className="signing">
          <Link to="/TourGuideHome/TourGuideProfile" className="text-white font-medium hover:underline">My Profile</Link>
        </nav>
      </header>
      ...
      <main className="flex-grow flex items-center justify-center mt-32 w-full">
        ...
        <div className="lg:w-1/3 flex-shrink-0 flex flex-col items-center">
            {/* Set a fixed square size for the image */}
            <img
              src={details.Picture}
              alt={details.Name || "Image"}
               className="w-72 h-72 object-cover rounded"
            />
            {/* Display Attended or Not Attended status */}
            {attendedStatus !== null && (
            ...
            <div className="mt-6">
              ...
              {type === "itinerary" && (
                <div className="mt-6">
                  ...
                  {errorTourGuide && (
                    <p className="text-red-500 mt-2">{errorTourGuide}</p>
                  )}
                </div>
              )}
            </div>
    ...
    <footer className="bg-brandBlue shadow dark:bg-brandBlue m-0 mt-20">
