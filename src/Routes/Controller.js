...
const bookflight = async (req, res) => {
    try {
        const { Tourist_Email , Flight_Name , Price ,Departure_Date, Return_Date, Departure , Arrival ,Duration} = req.body;

        if (!Tourist_Email || !Flight_Name || !Price || !Departure || !Arrival || !Duration || !Departure_Date || !Return_Date) {
            return res.status(400).json({ message: "Tourist Email, Flight Name, Price,,Departure_Date ,Return_Date, Departure, Arrival and Duration are required." });
        }

        // check if if the booking exists
        const existingBooking = await flightm.findOne({ Tourist_Email, Flight_Name, Price,Departure_Date ,Return_Date, Departure, Arrival, Duration });
        if (existingBooking) {
        ...
        // make the booking
        const newBooking = new flightm({ Tourist_Email, Flight_Name, Price,Departure_Date ,Return_Date, Departure, Arrival, Duration });
        await newBooking.save();
